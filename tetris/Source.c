...
void Show_Text(int k) {
    ...
    if (k == 1) {
        ...
        glScalef(15, 15, 0);
        glColor3f(1, 0, 0);
        Text_Out(tex_textID, "ÑÒÀÐÒ");
    }
    else if (k == 2) {
        glTranslatef(123, 57, 0);
        glScalef(8, 8, 0);
        glColor3f(1, 1, 1);
        Text_Out(tex_textID, "ÐÅÑÒÀÐÒ");
        glTranslatef(0.75, 3.7, 0);
        glColor3f(1, 1, 1);
        Text_Out(tex_textID, "ÏÀÓÇÀ");
        glTranslatef(0, -7.4, 0);
        glColor3f(1, 1, 1);
        Text_Out(tex_textID, "ÂÛÉÒÈ");

        glTranslatef(-1.0, 17.55, 0);
        glColor3f(0, 0, 0);
        Text_Out(tex_textID, "C×¨Ò: ");
        glPopMatrix();
    }
    glPopMatrix();
}
